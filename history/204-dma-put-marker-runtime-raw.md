# 204. DMA_PUT Marker Runtime Raw

## Purpose

Run the one authorized ready-edge host4 browser runtime to produce the new marker field `nv_user_dma_put_writer_cpu_context`. This entry records the runtime launch only; preservation checks and marker extraction have not been run yet.

## Command

```sh
mkdir -p build-real-b3-matrix/browser-memory-watch-write-0x3a890-ready-edge-host4-user-dma-put-v1
...
- raw runtime completion

## Findings

The command exited with status `0`. No artifact inspection has been performed yet.

## Decision

Proceed to the required loop check before any preservation check, read-evidence combine, marker extraction, or other probe.

## Next Step

Run the bounded loop check, then perform preservation-first checks if approved.
